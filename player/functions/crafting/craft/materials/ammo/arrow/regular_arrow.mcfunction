...
execute store result score @s old_mat_count run data get block ~ 2 ~ Items[1].tag.items.materials.ammo.arrow.regular_arrow
execute store result score @s new_mat_count run scoreboard players add @s old_mat_count 4
execute store result block ~ 2 ~ Items[1].tag.items.materials.ammo.arrow.regular_arrow int 1 run scoreboard players get @s new_mat_count
