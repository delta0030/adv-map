...
execute store result score @s new_mat_count run scoreboard players remove @s old_mat_count 5
...
execute store result score @s old_mat_count run data get block 0 -1 0 Items[{Slot:1b}].tag.items.gear.armor.slime_set.slimy_helmet
execute store result score @s new_mat_count run scoreboard players add @s old_mat_count 1
execute store result block 0 -1 0 Items[1].tag.items.gear.armor.slime_set.slimy_helmet int 1 run scoreboard players get @s new_mat_count
item replace entity @s enderchest.1 from block 0 -1 0 container.1
setblock 0 -1 0 bedrock

tellraw @s {"text":"+1 Slimy Helmet","color":"green","italic":false}