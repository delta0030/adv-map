...
execute store result score @s old_mat_count run data get block ~ 2 ~ Items[1].tag.items.gear.armor.slime_set.slimy_helmet
execute store result score @s new_mat_count run scoreboard players add @s old_mat_count 1
execute store result block ~ 2 ~ Items[1].tag.items.gear.armor.slime_set.slimy_helmet int 1 run scoreboard players get @s new_mat_count
