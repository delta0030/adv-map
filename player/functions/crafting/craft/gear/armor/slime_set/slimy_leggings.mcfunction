...
execute store result score @s old_mat_count run data get block 0 -1 0 Items[{Slot:1b}].tag.items.gear.armor.slime_set.slimy_leggings
execute store result score @s new_mat_count run scoreboard players add @s old_mat_count 1
execute store result block 0 -1 0 Items[1].tag.items.gear.armor.slime_set.slimy_leggings int 1 run scoreboard players get @s new_mat_count
