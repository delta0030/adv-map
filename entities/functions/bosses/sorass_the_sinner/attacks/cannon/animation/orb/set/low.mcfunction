execute as @e[type=armor_stand,tag=orb_proj_temp] at @e[type=armor_stand,tag=orb_low] run spreadplayers ~ ~ 1 2.5 false @s

execute store result score %ypos sts_cannon as @e[type=armor_stand,tag=orb_low] run data get entity @s Pos[1] 1000
